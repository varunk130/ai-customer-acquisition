...
import type { ChatMessage } from "./useBeacon";
      ...
      <div className="max-h-[18rem] flex-1 space-y-3 overflow-y-auto px-5 py-4">
        {messages.length === 0 && !running && (
          <p className="py-6 text-center text-sm text-slate-500">
            Give Beacon a budget and a goal. Try the suggested prompt, or hit <span className="text-beacon">Run guided demo</span>.
          </p>
        )}
          ...
          {messages.map((m, i) =>
            m.role === "user" ? (
              <motion.div
                ...
                className="ml-auto max-w-[85%] rounded-2xl rounded-br-sm border border-beacon/20 bg-beacon/[0.07] px-3.5 py-2.5 text-[13.5px] text-slate-100"
              >
                {m.text}
              </motion.div>
            ) : (
              <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="flex max-w-[92%] gap-2.5">
                <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-white/10 bg-navy-700">
        ...
        {showTyping && (
          <div className="flex items-center gap-2.5">
            <span className="flex h-7 w-7 items-center justify-center rounded-full border border-white/10 bg-navy-700">
              <Logo showText={false} className="scale-75" />
            </span>
