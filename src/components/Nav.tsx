  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-navy/80 backdrop-blur-md">
      <nav className="container-px flex h-16 items-center justify-between" aria-label="Primary">
        <Link href="/" className="focusable rounded-md" aria-label="Beacon home">
          <Logo />
        </Link>
      ...
      {open && (
        <div className="border-t border-white/[0.06] bg-navy md:hidden">
          <div className="container-px flex flex-col py-3">
            {NAV_LINKS.map((l) => (
              <Link
                ...
                aria-current={isActive(l.href) ? "page" : undefined}
                className={`focusable rounded-md px-3 py-2.5 text-sm ${isActive(l.href) ? "text-beacon" : "text-slate-300"}`}
              >
                {l.label}
              </Link>
            ))}
