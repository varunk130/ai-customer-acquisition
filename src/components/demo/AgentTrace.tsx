...
const PHASE_LABEL: Record<number, string> = { 1: "Phase 1 · Build the plan", 2: "Phase 2 · Reallocate" };
        ...
        {event.toolCalls && event.toolCalls.length > 0 && (
          <div className="mt-2.5 space-y-1">
            {event.toolCalls.map((t, i) => (
              <div key={i} className="flex items-start gap-2 rounded-md border border-white/[0.06] bg-navy-950/50 px-2.5 py-1.5 font-mono text-[11px] text-slate-400">
                <Terminal className="mt-0.5 h-3 w-3 shrink-0 text-slate-600" aria-hidden />
                <span className="min-w-0">
                  <span className="text-indigo2">{t.tool}</span>
...
export function AgentTrace({ events, activeAgent, running }: { events: TraceEvent[]; activeAgent: AgentId | null; running: boolean }) {
  ...
  const visible = useMemo(() => {
    return events.filter((e) => {
      ...
      return !events.some((o) => o.phase === e.phase && o.agent === e.agent && o.ts >= e.ts && o.status !== "thinking");
    });
  }, [events]);
  ...
  return (
    <div className="panel flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-3.5">
        ...
        <span className={`flex items-center gap-1.5 text-[11px] ${running ? "text-beacon" : "text-slate-500"}`}>
          <span className={`h-1.5 w-1.5 rounded-full ${running ? "bg-beacon animate-pulse" : "bg-slate-600"}`} />
        ...
        {visible.length === 0 ? (
          <div className="flex h-full min-h-[20rem] flex-col items-center justify-center text-center">
            ...
            <p className="mt-4 max-w-xs text-sm text-slate-400">
              The trace animates here. Hit <span className="text-beacon">Run guided demo</span> or give Beacon a budget to start.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {groups.map(([phase, evs]) => (
              <div key={phase}>
